    ...
    <div className="admin-panel">
      ...
      <div className="admin-main">
        ...
        {tab === 'bookings' && <section className="admin-page">
          ...
          <div className="booking-list">
            {filteredBookings.map(booking => <article className="booking-row" key={booking.id}>
              ...
              <div className="booking-date"><strong>{formatDate(booking.date)}</strong><span>{booking.time || 'Time to confirm'}</span>{booking.assignedStaff && <small>With {booking.assignedStaff}</small>}{booking.address && <small>{booking.address}</small>}</div>
        ...
        {tab === 'settings' && owner && <section className="admin-page admin-settings">
          ...
          <div className="admin-section-heading"><div><p>Team</p><h2>Braiding team</h2></div></div>
          <p className="admin-hint">Denzhe handles nails, installations, lashes and wig styling/customisation herself — her time slot locks when she's booked. Braids, tribal braids and cornrows are shared between 2 braiders below; the booking system auto-assigns whichever one is free for that slot.</p>
          <div className="settings-split">
            <label>Braider 1 name<input value={settingsDraft.braiderNames[0]} onChange={event => setSettingsDraft({ ...settingsDraft, braiderNames: [event.target.value, settingsDraft.braiderNames[1]] })} /></label>
            <label>Braider 2 name<input value={settingsDraft.braiderNames[1]} onChange={event => setSettingsDraft({ ...settingsDraft, braiderNames: [settingsDraft.braiderNames[0], event.target.value] })} /></label>
          </div>
      ...
      {editing && <div className="service-editor-shell"><button className="service-editor-backdrop" onClick={() => setEditing(null)} /><form className="service-editor" onSubmit={saveService}><button className="editor-close" type="button" onClick={() => setEditing(null)}>Close</button><p className="admin-overline">Menu editor</p><h2>{serviceDraft.id ? 'Edit service' : 'Add service'}</h2><label>Service name<input required value={serviceDraft.name} onChange={event => setServiceDraft({ ...serviceDraft, name: event.target.value })} /></label><div className="editor-split"><label>Category<select value={serviceDraft.category} onChange={event => setServiceDraft({ ...serviceDraft, category: event.target.value as Service['category'] })}>{categories.filter(item => item.id !== 'all').map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label>Duration<input required value={serviceDraft.duration} onChange={event => setServiceDraft({ ...serviceDraft, duration: event.target.value })} /></label></div><label>Handled by<select value={serviceDraft.team || ''} onChange={event => setServiceDraft({ ...serviceDraft, team: (event.target.value || undefined) as Service['team'] })}><option value="">Auto (based on category)</option><option value="denzhe">Denzhe</option><option value="braiders">Braiding team (either of the 2 braiders)</option></select><small>Braids, Tribal Braids and Cornrows default to the braiding team; everything else defaults to Denzhe. Override here if a service (like a mixed combo) needs to be assigned differently.</small></label><label>Single price<input type="number" min="0" value={serviceDraft.price || ''} onChange={event => setServiceDraft({ ...serviceDraft, price: Number(event.target.value) })} /><small>Leave blank when using length prices below.</small></label><label>Length prices<textarea rows={4} value={variantText} onChange={event => setVariantText(event.target.value)} placeholder={'Short: 180\nMedium: 200\nLong: 220'} /><small>One option per line in the format Name: Price.</small></label><label>Image URL<input required value={serviceDraft.image} onChange={event => setServiceDraft({ ...serviceDraft, image: event.target.value })} /></label><label>Description<textarea required rows={3} value={serviceDraft.description} onChange={event => setServiceDraft({ ...serviceDraft, description: event.target.value })} /></label><label className="editor-check"><input type="checkbox" checked={serviceDraft.active !== false} onChange={event => setServiceDraft({ ...serviceDraft, active: event.target.checked })} />Show this service on the public website</label><button className="admin-primary" type="submit">Save service</button></form></div>}
    </div>
  );
}